...
%
% Default output:
%   Figures/Figure_Section2_Taxonomy.pdf
%
...
% panel literature. The annotations identify the unavailable object, the
% pre-validation procedure choice, and the later or indirect validation
% object for each empirical case.
...
panels = { ...
    struct('id', 'A', 'title', 'A. Delayed release / ragged edge', ...
    ...
    struct('id', 'D', 'title', 'D. Completed-panel evaluation', ...
        'short_title', 'D. Completed-panel evaluation', ...
...
for f = 1:numel(formats)
    ...
    file_i = fullfile(output_dir, sprintf('Figure_Section2_Taxonomy.%s', fmt));
...
WC(27:T, 1:4) = 0;
WC(:, split + 1:end) = 1;
% Keep the validation population structured and directly evaluable; the
% orange outlines below identify the held-out validation cells.
VC = false(T, N);
VC(6:6:34, split + 4:6:N - 2) = true;
VC = VC & WC == 1;
patterns.C.W = WC;
patterns.C.V = VC;
...
if nargin < 9 || isempty(line_width); line_width = 1.55; end
...
if ~isempty(label)
    mid = (p1 + p2) / 2 + offset;
    text(ax, mid(1), mid(2), label, 'FontSize', 9.8, ...
...
draw_arrow(ax, [x_axis y_first], [x_axis y_last], colors, '-', '', [], colors.accent, 1.25);
text(ax, x_axis - 0.015, (y_first + y_last) / 2, 'time', ...
    'FontSize', 9.8, 'HorizontalAlignment', 'center', ...
    'VerticalAlignment', 'middle', 'Rotation', 90, 'Interpreter', 'none');
for r = 1:numel(release_rows)
    y = box(2) + (release_rows(r) - 0.5) / T * box(4);
    plot(ax, [box(1) - 0.014 box(1)], [y y], '-', ...
        'Color', colors.ink, 'LineWidth', 0.8);
    text(ax, box(1) - 0.019, y, sprintf('t%d', r), ...
        'FontSize', 8.4, 'HorizontalAlignment', 'right', ...
        'VerticalAlignment', 'middle', 'Interpreter', 'none');
end
end
...
for m = 1:numel(tt)
    rectangle(ax, 'Position', [box(1) + (jj(m) - 1) * cw, box(2) + (tt(m) - 1) * ch, cw, ch], ...
        'FaceColor', 'none', 'EdgeColor', colors.validation, 'LineWidth', 1.60);
end
end

function draw_panel_a(ax, p, colors)
box = [0.08 0.17 0.82 0.60];
...
plot(ax, [box(1) box(1) + box(3)], [tau_y tau_y], '-', ...
    'Color', colors.accent, 'LineWidth', 1.65);
text(ax, box(1) + 0.020, tau_y + 0.030, 'decision vintage', ...
    'FontSize', 10.0, 'Color', colors.accent, 'VerticalAlignment', 'middle', ...
...
text(ax, u(1) + 0.047, u(2) + 0.020, 'cell to complete', ...
    'FontSize', 9.6, 'Color', colors.ink, 'BackgroundColor', colors.white, ...
    'Margin', 1.0, 'Interpreter', 'none');

draw_arrow(ax, [0.600 0.872], u + [-0.018 0.010], colors, '-', '', [], colors.accent, 2.0);
draw_arrow(ax, [0.845 0.872], u + [0.026 0.010], colors, '--', '', [], colors.feedback, 1.9);
text(ax, 0.555, 0.902, 'procedure choice', ...
    'FontSize', 10.1, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
    'BackgroundColor', colors.white, 'Margin', 1.2, 'Interpreter', 'none');
text(ax, 0.842, 0.902, 'later release / validation', ...
    'FontSize', 9.7, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
    'BackgroundColor', colors.white, 'Margin', 1.2, 'Interpreter', 'none');
end

function draw_panel_b(ax, p, colors)
box = [0.15 0.18 0.66 0.64];
draw_pattern(ax, p.W, box, colors);
...
for rr = p.release_rows
    ...
    plot(ax, [box(1) box(1) + box(3)], [y y], '-', ...
        'Color', colors.validation, 'LineWidth', 0.90);
end
text(ax, box(1) + box(3) / 2, box(2) + box(4) + 0.045, 'high-frequency operating grid', ...
    'FontSize', 10.2, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
    'Interpreter', 'none');
text(ax, box(1) + box(3) * 0.60, box(2) + box(4) - 0.040, 'lower-frequency release dates', ...
    'FontSize', 9.6, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
    'Color', colors.validation, 'BackgroundColor', colors.white, ...
    'Margin', 1.0, 'Interpreter', 'none');
rr = p.release_rows(3);
T = size(p.W, 1);
...
plot(ax, [x_br x_br], [y_low y_high], '-', 'Color', colors.feedback, 'LineWidth', 1.75);
plot(ax, [x_br - 0.010 x_br], [y_low y_low], '-', 'Color', colors.feedback, 'LineWidth', 1.75);
plot(ax, [x_br - 0.010 x_br], [y_high y_high], '-', 'Color', colors.feedback, 'LineWidth', 1.75);
text(ax, x_br + 0.064, (y_low + y_high) / 2, 'aggregation restriction', ...
    'FontSize', 9.5, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
    'BackgroundColor', colors.white, 'Margin', 1.0, 'Interpreter', 'none');
end

function draw_panel_c(ax, p, colors)
box = [0.08 0.17 0.80 0.63];
draw_pattern(ax, p.W, box, colors);
...
plot(ax, [sep_x sep_x], [box(2) box(2) + box(4)], '-', ...
    'Color', colors.white, 'LineWidth', 1.9);
plot(ax, [sep_x sep_x], [box(2) box(2) + box(4)], '-', ...
    'Color', colors.ink, 'LineWidth', 1.0);
plot(ax, [box(1) + 0.010 sep_x - 0.010], [box(2) + box(4) + 0.016 box(2) + box(4) + 0.016], '-', ...
    'Color', colors.accent, 'LineWidth', 2.0);
plot(ax, [sep_x + 0.010 box(1) + box(3) - 0.010], [box(2) + box(4) + 0.016 box(2) + box(4) + 0.016], '-', ...
    'Color', colors.validation, 'LineWidth', 2.0);
text(ax, box(1) + 0.25, box(2) + box(4) + 0.035, 'target population', ...
    'FontSize', 10.4, 'HorizontalAlignment', 'center', ...
    'Color', colors.accent, 'Interpreter', 'none');
text(ax, sep_x + 0.20, box(2) + box(4) + 0.035, 'validation population', ...
    'FontSize', 10.3, 'HorizontalAlignment', 'center', ...
    'VerticalAlignment', 'middle', 'Color', colors.validation, ...
    'Interpreter', 'none');
text(ax, sep_x + 0.21, box(2) + 0.055, 'held-out validation cells', ...
...
box1 = [0.07 0.22 0.34 0.52];
box2 = [0.60 0.22 0.34 0.52];
...
text(ax, box1(1) + box1(3) / 2, header_y, 'decision-date vintage', ...
    'FontSize', 10.0, 'HorizontalAlignment', 'center', 'Interpreter', 'none');
text(ax, box2(1) + box2(3) / 2, header_y, 'completed panel', ...
    'FontSize', 10.0, 'HorizontalAlignment', 'center', 'Interpreter', 'none');
loss_inset = 0.010;
...
plot(ax, loss_x, loss_y, '-', 'Color', colors.feedback, 'LineWidth', 2.65);
text(ax, box2(1) + box2(3) / 2, box2(2) + 0.075, 'target-object loss', ...
    'FontSize', 11.0, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
    'Color', colors.feedback, 'BackgroundColor', colors.white, 'Margin', 1.1, ...
    'Interpreter', 'none');
text(ax, axis_label_x(box1), box1(2) + box1(4) / 2, 'time', ...
    'FontSize', 9.9, 'HorizontalAlignment', 'center', ...
    'VerticalAlignment', 'middle', 'Rotation', 90, 'Interpreter', 'none');
text(ax, 0.50, axis_label_y(box1), 'units', ...
    'FontSize', 9.9, 'HorizontalAlignment', 'center', ...
    'VerticalAlignment', 'middle', 'Interpreter', 'none');

draw_arrow(ax, [0.435 arrow_y], [0.575 arrow_y], colors, '-', '', [], colors.accent, 2.15);
text(ax, 0.505, arrow_y + 0.080, 'completion rule', ...
    'FontSize', 10.3, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', ...
